...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for act in self.all_actions:
            new_node = PgNode_a(act)
            if new_node.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(new_node)
                for pnode in self.s_levels[level]:
                    if pnode in new_node.prenodes:
                        new_node.parents.add(pnode)
                        pnode.children.add(new_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for act in node_a1.action.effect_add:
            if act in node_a2.action.effect_rem:
                return True
        for act in node_a2.action.effect_add:
            if act in node_a1.action.effect_rem:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for act in node_a1.action.effect_add:
            if act in node_a2.action.precond_neg:
                return True
        for act in node_a1.action.effect_rem:
            if act in node_a2.action.precond_pos:
                return True
        for act in node_a2.action.effect_add:
            if act in node_a1.action.precond_neg:
                return True
        for act in node_a2.action.effect_rem:
            if act in node_a1.action.precond_pos:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for pre1 in node_a1.parents:
            for pre2 in node_a2.parents:
                if pre1.is_mutex(pre2) or pre2.is_mutex(pre1):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for act1 in node_s1.parents:
            for act2 in node_s2.parents:
                if not act1.is_mutex(act2) or not act2.is_mutex(act1):
                    return False
        return True
        ...
        for g in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    if g == state.symbol and state not in visited and state.is_pos:
                        visited.add(state)
                        level_sum += level
                        flag = True
                        break
